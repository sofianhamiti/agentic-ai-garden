    ...
    <section class="header-section">
      <div class="container">
        ...
        <p class="page-description">
          Deepen your expertise with curated blogs, videos, and workshops
        </p>
      </div>
    </section>
        ...
        <div class="content-with-sidebar">
          <div class="content-container">
            ...
            <div class="filters-area">
              <FilterSidebar :resource-count="filteredContent.length" />
            </div>
...
watch(() => filteredContent.value, (newVal, oldVal) => {
  console.log('[LEARN] Filtered content changed:', newVal.length, 'items (previously', oldVal?.length || 0, 'items)');
...
function handleTabClick(tabValue) {
  console.log(`[LEARN] Tab clicked: ${tabValue}`);
  
  if (filters.value.type !== tabValue) {
    console.log(`[LEARN] Updating tab from ${filters.value.type} to ${tabValue}`);
    filters.value.type = tabValue;
    console.log('[LEARN] Tab change complete');
  }
}

// Initialize tab logging
onBeforeMount(() => {
  console.log('[LEARN] Component mounting, initial tab value:', filters.value.type);
  
  // Set page to 'learn'
  filters.value.page = 'learn';
  console.log('[LEARN] Set page filter to:', filters.value.page);
  
  // Validate the filter type on mount
  const validTypes = ['all', 'blogs', 'videos', 'workshops'];
  if (!validTypes.includes(filters.value.type)) {
    console.log(`[LEARN] Resetting invalid initial filter type '${filters.value.type}' to 'all'`);
...
watch(() => filters.value, (newVal) => {
  console.log('[LEARN] Current filter state:', {
    type: newVal.type,
...
onMounted(async () => {
  console.log('[LEARN] Component mounted, loading content');
  
  // Read type from URL query parameter if present
  const typeParam = route.query.type;
  if (typeParam && ['all', 'blogs', 'videos', 'workshops'].includes(typeParam)) {
    console.log(`[LEARN] Setting type from URL parameter: ${typeParam}`);
  ...
  await nextTick();
  console.log(`[LEARN] Content loaded and processed. Filtered content: ${filteredContent.value.length} items`);
})
